...
const baseUrl = 'https://stg.orca.redisea.com';
...
export default function (data) {
    ...
    const companyCrewId = 'd6dda1b2-4398-4f30-815c-edcd98bcb807';
    const vesselId = '0f181032-d7b6-4cb7-9daf-2e6812de7bff';
    const year = 2024;
    const month = 5;
    const rateCurrency = 16064;
    const totalDaysInMonth = 30;
    const age = 0;
	const page= 3;

    // Construct the URL with the provided parameters
    const payslipUrl = `${baseUrl}/v1/company/payroll/vessel/${vesselId}/crew/payslip?year=${year}&month=${month}&page=${page}`;
    ...
    if (matchingPayslipItem) {
		...
		//console.log('Matching Payslip Item:', matchingPayslipItem);
		...
		// console log only show when fail
        /*for (const [key, value] of Object.entries(results)) {
            console.log(`${key}: ${value ? 'PASS' : 'FAIL'}`);
            if (!value) {
                const expectedKey = key.split(' ')[0];
                console.log(`Expected: ${eval(expectedKey)}, Actual: ${matchingPayslipItem[expectedKey]}`);
            }
        }*/
